...
export default class NeteaseProvider extends BaseProvider {
   ...
   */
  url(id, br = 320) {
    // Modern netease ignores `br` alone and returns empty urls unless a
    // `level` is also sent. Map the requested bitrate to a level so free
    // songs actually resolve (VIP-only tracks still come back empty).
    let level = 'standard';
    let rate = Number(br) || 320;
    if (rate >= 480) level = 'lossless';
    else if (rate >= 320) level = 'exhigh';
    else if (rate >= 192) level = 'higher';
    else if (rate >= 128) level = 'standard';
    else level = 'low';
    return {
      method: 'POST',
