...
const POOL_TARGET = 5;
const ACCOUNT_RETRIES = 3;

async function withRetry(fn, label, tries = ACCOUNT_RETRIES) {
  let lastErr;
  for (let i = 0; i < tries; i++) {
    try {
      ...
      lastErr = e;
      if (i < tries - 1) {
        const wait = 1500 * (i + 1) + Math.random() * 1000;
        logSys(chalk.yellow(`${label} attempt ${i + 1} failed (${e.message}) — retrying in ${Math.round(wait)}ms`));
...
async function fillPool() {
  ...
  try {
    for (let i = 0; i < needed; i++) {
      try {
        const acc = await withRetry(() => createAccountRaw(), "pool fill");
        pool.push(acc);
...
async function createAccount() {
  ...
  const acc = await withRetry(() => createAccountRaw(), "on-demand account");
  fillPool().catch(() => {});
  return acc;
}
...
async function createAccountRaw() {
  ...
  const regRes = await withRetry(async () => {
    const r = await fetchWithTimeout("https://api.mail.tm/accounts", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ address: email, password: mailPassword }),
    });
    if (!r.ok) throw new Error(`Mail.tm register HTTP ${r.status}`);
    return r;
  }, "mail.tm register");
  if (!regRes.ok) throw new Error("Failed to register Mail.tm mailbox");

  const tokenRes = await withRetry(async () => {
    const r = await fetchWithTimeout("https://api.mail.tm/token", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ address: email, password: mailPassword }),
    });
    if (!r.ok) throw new Error(`Mail.tm token HTTP ${r.status}`);
    return r;
  }, "mail.tm token");
...
app.use((req, res, next) => {
  ...
  // Account creation (temp-mail verification) can idle for a while; a 30s
  // socket timeout killed sessions mid-stream once the response had already
  // started, showing up as IncompleteRead on the relay. Give createSession
  // room to breathe; keep other routes snappy.
  const long = (req.url || "").indexOf("/cloud/v1/createSession") === 0;
  req.setTimeout(long ? 300_000 : 30_000, () => {
    if (res.headersSent) {
      try { res.end(); } catch {}
    } else {
