...
rem ---- 2. Task Scheduler ONLOGON (hidden: runs the VBS, no cmd window) ----
set "VBSKEEPER=%~dp0start-keeper-hidden.vbs"
if exist "%VBSKEEPER%" (
  schtasks /Create /F /TN "ChalkleKeeper" /TR "wscript.exe \"%VBSKEEPER%\"" /SC ONLOGON /RL LIMITED >nul 2>&1
) else (
